...
class DSNET {
  ...
  constructor(accessToken: string, baseUrl = 'https://panel.dsnet.agh.edu.pl/api/beta') {
    this.accessToken = accessToken;
    this.baseURL = baseUrl;
  }

  /** Get dormitory types array, no idea if this actually ever changes */
  async getDormitoryTypes(): Promise<types.DormitoryType[]> {
  ...
  async getDormitoryList(): Promise<types.Dormitory[]> {
  ...
  async getDormitoryFloors(dormitoryID: number): Promise<types.Floor[]> {
  ...
  async getRoomsAtFloor(floorID: number): Promise<types.Room[]> {
  ...
  async getReservations(): Promise<types.Reservation[]> {
    ...
    const mappedRes = reservations.data.map((res) => {
      ...
      if(parsedTo.getHours() < 6) parsedTo.setDate(parsedTo.getDate() + 1);
      return {
  ...
  async getUserInfo(): Promise<types.UserInfo> {
